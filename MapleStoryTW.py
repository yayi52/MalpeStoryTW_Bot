...
CHARACTER_URL = config["maple"]["CHARACTER_URL"]
OCID_URL = config["maple"]["OCID_URL"]
POPULARITY_URL = config["maple"]["POPULARITY_URL"]
...
def get_character_data(name):
    ...
    response = requests.get(CHARACTER_URL, headers=headers, params={"ocid": get_ID(name)})
    if response.status_code == 200:
        data = response.json()
        return data
    else:
        ...
        return None
##名聲
def get_character_popularity(name):
    response = requests.get(POPULARITY_URL, headers=headers, params={"ocid": get_ID(name)})
    if response.status_code == 200:
        data = response.json()
        return data
    else:
...
async def character(interaction: discord.Interaction, name: str):
    ...
    data_popularity = get_character_popularity(name)
    ...
    if channel and get_data:
        ...
        if data_popularity["popularity"] > 10:
            embed.add_field(name="人氣不錯喔", value="名聲：" + str(data_popularity["popularity"]), inline=True)
        else:
            embed.add_field(name="人氣再加油", value="名聲：" + str(data_popularity["popularity"]), inline=True)
        # 加上角色頭像
